...
export class StandardDashboardComponent implements OnInit {
    ...
    getDashboardPermissions() {
        this.apiService.getTestLandingPage().subscribe(data => {
            this.ConfigTableBodyData = data;
            console.log("Dashboard Permissions data -> ", data);
            this.rerender();
        });
    }

    onCancel(dismissMethod: string): void {
        console.log('Canecl ', dismissMethod);
    }
    ...
    save(data, event) {
        ...
        this.apiService.setLandingPagePermission(data.ca_bsi_user_id, event.target.checked).subscribe(data => {
